...
def load_and_process_all(files=None, bypass_cache=False):
    ...
    def fetch_one(name, url):
        ...
        try:
            ...
            if name == "Inaproc":
                ...
                s_mask = df['ID SIRUP'] != 'MISSING'
                k_mask = df['Kode Paket'] != 'MISSING'
                df['SOURCE_KEY'] = 'RAND_' + pd.Series(np.random.randint(1000000, 9999999, size=len(df))).astype(str)
                df.loc[s_mask, 'SOURCE_KEY'] = 'SIRUP_' + df.loc[s_mask, 'ID SIRUP']
                df.loc[k_mask, 'SOURCE_KEY'] = 'KP_' + df.loc[k_mask, 'Kode Paket']
                df.loc[s_mask & k_mask, 'SOURCE_KEY'] = df.loc[s_mask & k_mask, 'ID SIRUP'] + "_" + df.loc[s_mask & k_mask, 'Kode Paket']
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        results = list(executor.map(lambda x: fetch_one(*x), urls.items()))
    
    for name, df, stat, dups in results:
        raw[name], stats[name], duplicates[name] = df, stat, dups
...
def build_master(raw):
    ...
    ina = raw["Inaproc"]
    master['Inaproc_NK'] = master['Nilai Kontrak']
    master['Inaproc_Rekanan'] = master['Rekanan']
    master['Matched Inaproc'] = False
    master['Match Method'] = "None"

    if not ina.empty:
        # Pre-calculate Inaproc Lookups (Vectorized)
        ina_sirup = ina[ina['ID SIRUP'] != 'MISSING'].groupby('ID SIRUP').agg({
            'nk_c': 'sum', 
            'Nama Rekanan': lambda x: '; '.join(x.astype(str).unique())
        })
        
        ina_name = ina[ina['norm_name'] != ''].groupby('norm_name').agg({
            'nk_c': 'sum', 
            'Nama Rekanan': lambda x: '; '.join(x.astype(str).unique())
        })

        # Priority 1: SIRUP Match (Vectorized Mapping)
        s_map_nk = ina_sirup['nk_c'].to_dict()
        s_map_rek = ina_sirup['Nama Rekanan'].to_dict()
        
        sirup_match_mask = master['ID SIRUP'].isin(s_map_nk.keys()) & (master['ID SIRUP'] != 'MISSING')
        master.loc[sirup_match_mask, 'Inaproc_NK'] = master.loc[sirup_match_mask, 'ID SIRUP'].map(s_map_nk)
        master.loc[sirup_match_mask, 'Inaproc_Rekanan'] = master.loc[sirup_match_mask, 'ID SIRUP'].map(s_map_rek)
        master.loc[sirup_match_mask, 'Matched Inaproc'] = True
        master.loc[sirup_match_mask, 'Match Method'] = "SIRUP"

        # Priority 2: Name Match (Vectorized Mapping for those not yet matched)
        n_map_nk = ina_name['nk_c'].to_dict()
        n_map_rek = ina_name['Nama Rekanan'].to_dict()
        
        name_match_mask = (~master['Matched Inaproc']) & master['norm_name'].isin(n_map_nk.keys()) & (master['norm_name'] != '')
        master.loc[name_match_mask, 'Inaproc_NK'] = master.loc[name_match_mask, 'norm_name'].map(n_map_nk)
        master.loc[name_match_mask, 'Inaproc_Rekanan'] = master.loc[name_match_mask, 'norm_name'].map(n_map_rek)
        master.loc[name_match_mask, 'Matched Inaproc'] = True
        master.loc[name_match_mask, 'Match Method'] = "Nama"
...
# PART 5: CHARTS AND LIST VIEW
        ...
        with t3:
            ...
            if not rek_df.empty:
                ...
                # Create formatted labels for charts
                rek_stats['Label Kontrak'] = rek_stats['Total Kontrak'].apply(format_idr)
                
                # Simplified Layout: Single column for Value Analysis
                fig_rek_val = px.bar(
                ...
                fig_rek_val.update_traces(textposition='inside', textfont_size=12)
                fig_rek_val.update_layout(
                    yaxis={'categoryorder':'total ascending'}, 
                    xaxis={'title': 'Total Kontrak (Rp)'},
                ...
                disp_rek = rek_stats[['Rekanan', 'Jumlah Paket', 'Total Kontrak']].copy()
